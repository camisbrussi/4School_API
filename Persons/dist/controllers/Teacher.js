...
class TeacherController {
    ...
    async indexFilter(req, res) {
        ...
        try {
            ...
            if (status_id)
                whereTeacher.status_id = status_id;

            if (name)
                whereTeacherPerson.name = {[_sequelize.Op.substring]: name};
            if (cpf)
                whereTeacherPerson.cpf = cpf;
            if (yearBirth)
                whereTeacherPerson.birth_date = {[_sequelize.Op.between]: [yearBirth + "-01-01", yearBirth + "-12-31"]}
