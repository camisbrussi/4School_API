...
class TeamController {
    ...
    async storeStudents(req, res) {
        ...
        try {
            ...
            if (erros.length)
                ...
                logger.info({
                    level: 'Info',
                    message: `Estudantes registrados com sucesso`,
                    label: `Registro - ${userLogged.login}@${userLogged.id}`,
                });
    ...
    async update(req, res) {
        ...
        try {
            ...
            const {teacher_id, isActive, name, year} = req.body;
            ...
            if (erros.length) {
                ...
                let status_id = isActive == 1 ? process.env.TEAM_STATUS_ACTIVE : process.env.TEAM_STATUS_INACTIVE;

                const newData = await team.update({teacher_id,status_id, name, year});
                logger.info({
                    level: 'info',
    ...
    async filterStudents(req, res) {
        try {
            ...
            logger.error({
                level: 'error',
                message: e.errors.map((err) => err.message),
                label: `Erro ao filtrar Estudantes`,
            });
        }
    }

    async teamsTeacher(req, res) {
        ...
        try {
            ...
            if (!id) {
                return res.status(400).json({
                    errors: ['ID do professor não encontrado'],
                });
            }

            const teacher = await Teacher.findOne({where: {person_id: id}});
            if (!teacher) {
                return res.status(400).json({
